...
FROM
    ...
    JOIN dim_product p ON m.product_name = p.name AND m.product_category = p.category
    JOIN dim_store st ON m.store_name = st.name AND m.store_location = st.location
